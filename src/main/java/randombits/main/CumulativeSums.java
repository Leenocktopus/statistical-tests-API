...
public class CumulativeSums {
    ...
    public static double[] test(String sequence) {
        ...
        double forwardZ = Arrays.stream(forward).map(Math::abs).max().orElse(0);
        double backwardZ = Arrays.stream(backward).map(Math::abs).max().orElse(0);
        return new double[]{phi(n, forwardZ), phi(n, backwardZ)};
    }

    public static double phi(int n, double z) {
        double e1 = 0, e2 = 0;
        int start = (int) Math.floor(4 * Math.floor(-n / z) + 1);
        int end = (int) Math.floor(4 * Math.floor(n / z) - 1);

        for (double i = start; i < end + 1; i++) {
            e1 += FunctionUtils.normalD(((4 * i + 1) * z) / Math.sqrt(n));
            e1 -= FunctionUtils.normalD(((4 * i - 1) * z) / Math.sqrt(n));
        }
        ...
        for (double i = start; i < end + 1; i++) {
            e2 += FunctionUtils.normalD(((4 * i + 3) * z) / Math.sqrt(n));
            e2 -= FunctionUtils.normalD(((4 * i + 1) * z) / Math.sqrt(n));
        }
        return (1 - e1) + e2;
    }
